...
pub async fn handle_messages(
    ...
    tracing::info!("[{}] Model Routing: Primary={}, Candidates={:?}", trace_id, route_plan.primary, model_candidates);
    ...
    for (model_index, candidate_model) in model_candidates.iter().enumerate() {
        ...
        for attempt in 0..max_attempts {
            ...
            Err(e) => {
                if !is_last_model {
                    tracing::warn!(
                        "[{}] Failed to get token for model {} (strategy attempt {}/{}), trying next candidate...",
                        trace_id,
                        mapped_model,
                        model_index + 1,
                    ...
                    switched_model = true;
                    break; // Break inner retry loop to continue outer candidate loop
                }
